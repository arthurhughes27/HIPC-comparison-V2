# A script to perform within-vaccine predictions based on sequential transcriptomics data over time without including TBA modules
...
# In this file, we use XGboost models to make predictions in a nested cross-validation framework within each vaccine's data
...
# Path to sequential prediction sets list
p_load_sequential_prediction_sets_withoutTBA <- fs::path(processed_data_folder,
                                                         "sequential_prediction_sets_list_withoutTBA.rds")

# Load sequential prediction sets list
sequential_prediction_sets_withoutTBA = readRDS(p_load_sequential_prediction_sets_withoutTBA)
...
# prediction_set_list = sequential_prediction_sets_withoutTBA
# vaccine = "Yellow Fever (LV)"
# set_name = "Day 7"
# n_outer = 10
# n_inner = 5
...
# verbose = TRUE
  ...
  X_mat$gender <- droplevels(X_mat$gender)
  gender_dummies <- model.matrix(~ gender - 1, data = X_mat)
  # Drop the original gender column
  X_no_gender <- X_mat[, setdiff(names(X_mat), "gender"), drop = FALSE]
  
  # Combine everything
  X_encoded <- cbind(X_no_gender, gender_dummies)
  
  # Convert to matrix if needed (XGBoost prefers a numeric matrix)
  X_mat <- as.matrix(X_encoded)
  ...
  for (fold_idx in seq_along(folds)) {
    ...
    ctrl_rand <- trainControl(method = "cv", number = 5, search = "random")
    ...
    fit <- train(
      ...
      y = y_train,
      method = "xgbTree",
      trControl = ctrl_rand,      # your trainControl object
      tuneLength = 500,
      # tuneGrid = tune_grid,
      metric = "RMSE",
      verbose = F,
...
prediction_results_all_sequential_withClinical_withoutTBA_xgboost <- vector("list", length(sequential_prediction_sets_withoutTBA))
names(prediction_results_all_sequential_withClinical_withoutTBA_xgboost) <- names(sequential_prediction_sets_withoutTBA)

# For each vaccine
for (vac in c("Yellow Fever (LV)")) {
  # initialize inner list with same names as the predictor-sets for this vaccine
  set_names <- names(sequential_prediction_sets_withoutTBA[[vac]])
  prediction_results_all_sequential_withClinical_withoutTBA_xgboost[[vac]] <- vector("list", length(set_names))
  names(prediction_results_all_sequential_withClinical_withoutTBA_xgboost[[vac]]) <- set_names
  ...
  for (set_name in set_names) {
    ...
    # store the result
    prediction_results_all_sequential_withClinical_withoutTBA_xgboost[[vac]][[set_name]] <- res
...
# Save these results
p_prediction_results_all_sequential_withClinical_withoutTBA_xgboost = fs::path(
  "output",
  "results",
  "prediction",
  "prediction_results_all_sequential_withClinical_withoutTBA_xgboost.rds"
)

saveRDS(prediction_results_all_sequential_withClinical_withoutTBA_xgboost,
        file = p_prediction_results_all_sequential_withClinical_withoutTBA_xgboost)
...
# names(prediction_results_all_sequential_withClinical_withoutTBA_xgboost) <- names(sequential_prediction_sets_withoutTBA)
# # For each vaccine
# for (vac in names(sequential_prediction_sets_withoutTBA)) {
#   # initialize inner list with same names as the predictor-sets for this vaccine
#   set_names <- names(sequential_prediction_sets_withoutTBA[[vac]])[-1]
#   prediction_results_all_sequential_withClinical_withoutTBA_xgboost[[vac]] <- vector("list", length(set_names))
#   names(prediction_results_all_sequential_withClinical_withoutTBA_xgboost[[vac]]) <- set_names
#   
#   # For each predictor set
...
prediction_results_all_sequential_withClinical_withoutTBA_xgboost = readRDS(p_prediction_results_all_sequential_withClinical_withoutTBA_xgboost)

# --- 1. Build vaccine x set grid and extract Rspearman + sRMSE in one pass ----
vaccines <- names(prediction_results_all_sequential_withClinical_withoutTBA_xgboost)
...
grid <- expand.grid(vaccine = "Yellow Fever (LV)",
                    set     = all_sets,
...
for (i in seq_len(n)) {
  ...
  if (is.null(prediction_results_all_sequential_withClinical_withoutTBA_xgboost[[vac]]))
    next
  res_set <- prediction_results_all_sequential_withClinical_withoutTBA_xgboost[[vac]][[setn]]
...
heatmap_plot_R <- ggplot(plot_df, aes(x = set, y = vaccine, fill = Rs)) +
  ...
  labs(x = "Predictor set",
       ...
       title = expression(paste(
         "Spearman ", rho, " by vaccine and predictor set - XGboost"
       ))) +
...
heatmap_plot_sRMSE <- ggplot(plot_df, aes(x = set, y = vaccine, fill = sRMSE)) +
  ...
  coord_fixed(ratio = 1) +   # square tiles
  labs(x = "Predictor set", y = "Vaccine", title = "Standardised RMSE by vaccine and predictor set - XGboost") +
  theme_minimal(base_size = 20) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    plot.title = element_text(size = 25, hjust = 0.5),
...
combined <- heatmap_plot_R_mod / heatmap_plot_sRMSE_mod +
  ...
  plot_annotation(
    title = "Evaluation metrics of CV predictions - XGboost",
...
# --- Save figure ---
ggsave(
  filename = "evaluation_combined_sequential_withClinical_withoutTBA_xgboost.pdf",
...
# # --- Save figure ---
# ggsave(
#   filename = "evaluation_combined_sequential_withoutClinical_withoutTBA_xgboost.pdf",
#   path = prediction_figures_folder,
#   plot = combined,
#   width = 40,
#   height = 25,

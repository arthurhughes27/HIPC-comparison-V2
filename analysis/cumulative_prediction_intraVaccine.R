...
prediction_results_all_cumulative_withClinical <- vector("list", length(cumulative_prediction_sets))
names(prediction_results_all_cumulative_withClinical) <- names(cumulative_prediction_sets)
...
for (vac in names(cumulative_prediction_sets)) {
  ...
  prediction_results_all_cumulative_withClinical[[vac]] <- vector("list", length(set_names))
  names(prediction_results_all_cumulative_withClinical[[vac]]) <- set_names
  ...
  for (set_name in set_names) {
    ...
    # store the result
    prediction_results_all_cumulative_withClinical[[vac]][[set_name]] <- res
...
saveRDS(prediction_results_all_cumulative_withClinical, file = p_prediction_results_all_cumulative_withClinical)

# Do the same prediction task but remove clinical variables from the prediction

prediction_results_all_cumulative_withoutClinical <- vector("list", length(cumulative_prediction_sets))
names(prediction_results_all_cumulative_withoutClinical) <- names(cumulative_prediction_sets)
# For each vaccine
for (vac in names(cumulative_prediction_sets)) {
  # initialize inner list with same names as the predictor-sets for this vaccine
  set_names <- names(cumulative_prediction_sets[[vac]])[-1]
  prediction_results_all_cumulative_withoutClinical[[vac]] <- vector("list", length(set_names))
  names(prediction_results_all_cumulative_withoutClinical[[vac]]) <- set_names
  ...
  for (set_name in set_names) {
    ...
    # store the result
    prediction_results_all_cumulative_withoutClinical[[vac]][[set_name]] <- res
...
saveRDS(prediction_results_all_cumulative_withoutClinical, file = p_prediction_results_all_cumulative_withoutClinical)
...
vaccines <- names(prediction_results_all_cumulative_withClinical)
...
for (i in seq_len(n)) {
  ...
  if (is.null(prediction_results_all_cumulative_withClinical[[vac]]))
    next
  res_set <- prediction_results_all_cumulative_withClinical[[vac]][[setn]]
...
# --- Save figure ---
ggsave(
  filename = "evaluation_combined_cumulative_withClinical.pdf",
...
# --- 1. Build vaccine x set grid and extract Rspearman + sRMSE in one pass ----
vaccines <- names(prediction_results_all_cumulative_withoutClinical)
...
for (i in seq_len(n)) {
  ...
  setn <- grid$set[i]
  # guard against missing vaccine or set
  if (is.null(prediction_results_all_cumulative_withoutClinical[[vac]]))
    next
  res_set <- prediction_results_all_cumulative_withoutClinical[[vac]][[setn]]
  if (is.null(res_set) || !is.list(res_set))
    next
  if (!("metrics" %in% names(res_set)))
    next
...
# --- Save figure ---
ggsave(
  filename = "evaluation_combined_cumulative_withoutClinical.pdf",
  path = prediction_figures_folder,
  plot = combined,
  width = 40,
  height = 25,

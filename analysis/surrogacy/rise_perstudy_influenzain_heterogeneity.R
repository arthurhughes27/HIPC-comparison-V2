...
hipc_merged_all_norm$age_group2 = ifelse(hipc_merged_all_norm$age_imputed <= 50, "young", "old")
...
study_names = hipc_merged_all_norm_filtered %>% 
  ...
  count(study_time_collected, study_accession) %>%
  filter(n >= 2) %>%
...
p_rise_results_list_influenzain_heterogeneity = fs::path(
  "output",
  "results",
  "surrogacy",
  "heterogeneity",
  paste0("rise_results_list_influenzain_", sex, "_", age_cat,".rds")
)

# Save the results
saveRDS(rise_results_list,
        file = p_rise_results_list_influenzain_heterogeneity)

# Read back in the results if necessary
rise_results_list = readRDS(p_rise_results_list_influenzain_heterogeneity)
...
surrogacy_figures_folder = fs::path("output", "figures", "surrogacy", "heterogeneity")

ggsave(
  filename = paste0("influenzain_", sex, "_", age_cat,"_surrogacy_weightedranks.pdf"),
...
for (tp in timepoints_to_retain) {
  ...
  p2 <- ggplot(rise_results_df_filtered, aes(x = u_y, y = u_s)) +
    ...
    labs(
      ...
      y = expression(U[S]),
      title = paste0(
        "Influenza (IN) in ",age_cat," ", sex,": Treatment effects on response vs marker across trials (Day ",
        tp,
        ": N trials = ",
  ...
  ggsave(
    paste0("influenzain_", sex, "_", age_cat,"_gene_grid_day", tp, ".pdf"),
    path = surrogacy_figures_folder,
    plot = p2,
    device = cairo_pdf,
    width = 37,
...
if (length(timepoints_to_retain) > 1) {
  main_title <- paste0("Surrogacy heatmap - Influenza (IN) in ", age_cat, " ", sex, ": top ",
                       n_markers ,
...
pdf_file <- surrogacy_figures_folder / paste0(
  "heatmap_surrogacy_influenzain_", sex, "_", age_cat,"_days",
...
grid.text(main_title, gp = gpar(fontsize = n_trials*1.5, fontface = "bold"))
popViewport()
